...
export default function TourDetailsPage() {
  ...
  const roadmapItems = [
    {
      day: lang === 'ru' ? 'День 1' : lang === 'en' ? 'Day 1' : '1-Kun',
      title: lang === 'ru' ? 'Прибытие, заселение в 4★ отель и вечерняя прогулка' : lang === 'en' ? 'Arrival, 4★ Hotel Check-in & Evening Walk' : 'Yetib kelish, 4★ mehmonxonaga joylashish va tanishuv sayri',
      desc: lang === 'ru' ? 'Комфортный трансфер, заселение в уютные номера, вечерняя прогулка по историческому центру и дегустация национальных блюд.' : lang === 'en' ? 'Comfortable transfer, check-in to boutique rooms, evening promenade through historic landmarks & welcome dinner.' : 'Konditsionerli transportda manzilga yetib borish, qulay xonalarga joylashish va shahar markazi bo\'ylab kechki sayr hamda milliy taomlar kechki ovqati.'
    },
    {
      day: lang === 'ru' ? 'День 2' : lang === 'en' ? 'Day 2' : '2-Kun',
      title: lang === 'ru' ? 'Экскурсия по древним памятникам и музеям' : lang === 'en' ? 'Guided Tour of Ancient Monuments & Museums' : 'Qadimiy obidalar, yodgorliklar va muzeylar bo\'ylab ekskursiya',
      desc: lang === 'ru' ? 'Посещение великих архитектурных ансамблей с профессиональным гидом, фотосессия и традиционный плов на обед.' : lang === 'en' ? 'Explore iconic architectural wonders with a licensed historian guide, photo sessions & festive lunch.' : 'Professional litsenziyali tarixchi gid hamrohligida eng mashhur me\'moriy obidalarni ziyorat qilish, tarixiy fotosessiyalar va tushlikda an\'anaviy to\'y oshi.'
    },
    {
      day: lang === 'ru' ? 'День 3' : lang === 'en' ? 'Day 3' : '3-Kun',
      title: lang === 'ru' ? 'Природные заповедники и культурные центры' : lang === 'en' ? 'Natural Scenic Spots & Cultural Centers' : 'Tabiat go\'shalari, tog\' manzaralari va madaniy maskanlar',
      desc: lang === 'ru' ? 'Поездка по живописным природным ландшафтам, отдых на свежем воздухе и знакомство с ремесленными мастерскими.' : lang === 'en' ? 'Scenic landscapes, fresh air relaxation, waterfalls or reserves, and artisan craft workshops.' : 'Viloyatning eng chiroyli tabiiy manzaralari, sharsharalar yoki qo\'riqxonalari bo\'ylab sayohat, ochiq havoda dam olish va milliy hunarmandchilik markazlari.'
    },
    {
      day: lang === 'ru' ? 'День 4' : lang === 'en' ? 'Day 4' : '4-Kun',
      title: lang === 'ru' ? 'Восточный базар, сувениры и свободное время' : lang === 'en' ? 'Oriental Bazaar, Souvenirs & Leisure Time' : 'Sharq bozori, esdalik sovg\'alari va erkin xaridlar vaqti',
      desc: lang === 'ru' ? 'Покупка восточных сладостей, специй, сухофруктов и сувениров ручной работы на колоритном базаре.' : lang === 'en' ? 'Shopping authentic handicrafts, dried fruits, spices, and souvenirs at the vibrant bazaar.' : 'Mashhur sharq bozorlarida esdalik sovg\'alar, quruq mevalar va hunarmandchilik buyumlarini xarid qilish, qiziqarli mahorat darslari.'
    },
    {
      day: lang === 'ru' ? 'День 5' : lang === 'en' ? 'Day 5' : '5-Kun',
      title: lang === 'ru' ? 'Завтрак, памятные фото и возвращение в Ташкент' : lang === 'en' ? 'Breakfast, Farewell Photos & Safe Return' : 'Ertalabki nonushta, xotira fotosuratlari va Toshkentga qaytish',
      desc: lang === 'ru' ? 'Сытный завтрак в отеле, памятные снимки и комфортное возвращение на автобусе или авиарейсе.' : lang === 'en' ? 'Delicious hotel breakfast, final group memories, and smooth return by bus or direct flight.' : 'Mehmonxonada to\'yimli nonushta, sayohatchilar bilan xotira rasmlari va qulay avtobus / samolyotda xavfsiz qaytish.'
    }
  ];
  ...
  return (
    <div className="pt-24 sm:pt-32 pb-20 max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
      ...
      <div className="flex items-center justify-between">
        <Link
          to="/tours"
          className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors p-2 rounded-xl hover:bg-slate-100 cursor-pointer"
        >
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs uppercase font-extrabold tracking-wider text-emerald-700 bg-emerald-50 px-3.5 py-1 rounded-full border border-emerald-200">
            {lang === 'ru' ? '5-Дневный VIP Тур «Все Включено»' : lang === 'en' ? '5-Day VIP All-Inclusive Tour' : '5 Kunlik VIP All-Inclusive Tur'}
          </span>
          <span className="text-xs font-bold text-amber-700 bg-amber-50 px-3 py-1 rounded-full border border-amber-200 flex items-center gap-1">
            ...
            <span>4.99 ({lang === 'ru' ? '3,400+ отзывов' : lang === 'en' ? '3,400+ reviews' : '3,400+ baho'})</span>
          </span>
        </div>

        <h1 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight leading-tight">
          {name} — {lang === 'ru' ? '5-Дневный VIP Тур' : lang === 'en' ? '5-Day VIP Journey' : '5 Kunlik Sayohati'}
        </h1>
            ...
            <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
              <div>
                <span className="text-xs text-emerald-300 font-bold uppercase tracking-wider block">
                  {lang === 'ru' ? 'Достопримечательность' : lang === 'en' ? 'Featured Landmark' : 'Diqqatga sazovor maskan'}
                </span>
        ...
        <div className="lg:col-span-5 bg-slate-50 border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-lg space-y-6">
          ...
          <div>
            <label className="text-xs font-black uppercase tracking-wider text-slate-500 block mb-2">
              {lang === 'ru' ? 'Выберите транспорт:' : lang === 'en' ? 'Choose Transport:' : 'Transport turini tanlang:'}
            </label>
            <div className="grid grid-cols-2 gap-2 p-1 bg-white rounded-2xl border border-slate-200">
              ...
              >
                ...
                <span>{lang === 'ru' ? 'Самолет' : lang === 'en' ? 'Flight' : 'Samolyot Parvozi'}</span>
              </button>
            </div>
            {durationText && (
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
            <div className="flex items-baseline justify-between mb-1">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                {lang === 'ru' ? '5-Дневный Пакет «Все Включено»:' : lang === 'en' ? '5-Day All-Inclusive Package:' : "5 Kunlik To'liq Paket:"}
              </span>
            ...
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              {lang === 'ru' ? 'Все расходы на 5 дней включены на 1 чел.' : lang === 'en' ? 'All 5-day expenses included per person' : '1 kishi uchun to\'liq 5 kunlik xarajatlar kiritilgan'}
            </p>
          </div>
          ...
          <div className="space-y-2.5">
            <span className="text-xs font-black uppercase tracking-wider text-slate-700 block">
              {lang === 'ru' ? 'В стоимость тура включено:' : lang === 'en' ? 'Included in this tour:' : 'Paket ichiga kiritilgan:'}
            </span>
          ...
          >
            {lang === 'ru' ? 'Забронировать Тур Сейчас' : lang === 'en' ? 'Book This Tour Now' : 'Turni Hoziroq Band Qilish (Bron)'}
          </button>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-200 text-xs text-slate-600">
            <span>{lang === 'ru' ? 'Есть вопросы?' : lang === 'en' ? 'Questions?' : 'Savollaringiz bormi?'}</span>
            <a
        ...
        <div className="lg:col-span-2 space-y-6">
          <div>
            <h2 className="text-2xl font-black text-slate-900 mb-3">
              {lang === 'ru' ? 'Подробная Информация о Туре' : lang === 'en' ? 'Detailed Tour Overview' : "Tur Haqida Batafsil Ma'lumot"}
            </h2>
          ...
          <div className="space-y-4 pt-4">
            <h3 className="text-lg font-black text-slate-900">
              {lang === 'ru' ? 'Программа Тура на 5 Дней (Маршрут)' : lang === 'en' ? '5-Day Tour Itinerary' : '5 Kunlik Sayohat Dasturi (Marshrut)'}
            </h3>
        ...
        <div className="space-y-4">
          <div className="p-6 rounded-3xl bg-slate-900 text-white space-y-5 shadow-xl">
            <h3 className="text-base font-black text-emerald-400 uppercase tracking-wider">
              {lang === 'ru' ? 'Ключевые Данные' : lang === 'en' ? 'Key Trip Facts' : 'Asosiy Ma\'lumotlar'}
            </h3>

            <div className="space-y-3.5 text-xs">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-slate-400">{lang === 'ru' ? 'Город / Регион:' : lang === 'en' ? 'City / Region:' : 'Viloyat / Shahar:'}</span>
                <span className="font-bold text-white">{name}</span>
              </div>

              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-slate-400">{lang === 'ru' ? 'Длительность тура:' : lang === 'en' ? 'Tour Duration:' : 'Tur Davomiyligi:'}</span>
                <span className="font-bold text-white">{lang === 'ru' ? '5 Дней / 4 Ночи' : lang === 'en' ? '5 Days / 4 Nights' : '5 Kun / 4 Kecha'}</span>
              </div>

              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-slate-400">{lang === 'ru' ? 'Текущая погода:' : lang === 'en' ? 'Current Temperature:' : 'Hozirgi Harorat:'}</span>
                <span className="font-bold text-emerald-400">{country.temp || '+25°C'}</span>
              </div>

              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-slate-400">{lang === 'ru' ? 'Лучший сезон:' : lang === 'en' ? 'Best Season:' : 'Eng Yaxshi Fasl:'}</span>
                <span className="font-bold text-white">{bestTime}</span>
              </div>

              <div className="flex items-center justify-between">
                <span className="text-slate-400">{lang === 'ru' ? 'Рейтинг:' : lang === 'en' ? 'Rating:' : 'Reyting:'}</span>
            ...
            >
              {lang === 'ru' ? 'Забронировать сейчас' : lang === 'en' ? 'Book Tour Now' : 'Hozir band qilish'}
            </button>
          </div>
        </div>
