  ...
  return (
    <section id="destinations" className="py-24 relative overflow-hidden bg-white border-t border-slate-200">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <img
          src="/images/landmarks/bukhara-kalon-minaret.png"
          alt="Bukhara Kalon Historic Complex"
          className="w-full h-full object-cover object-center scale-120 opacity-30"
        />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        <Card className="rounded-3xl sm:rounded-[36px] overflow-hidden border border-slate-200 shadow-2xl grid grid-cols-1 lg:grid-cols-12 mb-16 p-0">
          <div className="lg:col-span-7 relative min-h-[420px] sm:min-h-[520px] flex flex-col justify-between p-6 sm:p-8 overflow-hidden bg-slate-900 text-white">
            <div className="absolute inset-0 z-0">
              <img src={activePhotoUrl} alt={activePhotoTitle} className="w-full h-full object-cover filter brightness-95" />
            ...
            <div className="relative z-10 flex items-center justify-between gap-3">
              ...
              <button onClick={() => onOpenImageLightbox(activePhotoUrl)} className="px-3.5 py-1.5 rounded-full bg-slate-900/80 hover:bg-white text-white hover:text-slate-900 backdrop-blur-md border border-white/30 text-xs font-bold flex items-center gap-2 transition-all shadow-md cursor-pointer">
            ...
            {galleryPhotos.length > 1 && (
              <div className="relative z-10 my-4 flex flex-wrap gap-2">
                {galleryPhotos.map((photo, idx) => (
                  <button key={idx} onClick={() => setActivePhotoIdx(idx)} className={`px-3 py-1.5 rounded-xl text-xs font-bold backdrop-blur-md transition-all flex items-center gap-2 border cursor-pointer ${activePhotoIdx === idx ? 'bg-[#10b981] text-white border-[#10b981] shadow-md' : 'bg-black/50 text-slate-200 border-white/20'}`}>
            ...
            <div className="relative z-10 space-y-2">
              ...
              <h3 className="text-2xl sm:text-3xl font-black text-white">{countryName} — {countryTagline}</h3>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/90 shadow-2xs">
                  <div className="flex items-center gap-1.5 text-slate-500 text-xs font-bold mb-1"><Sun className="w-4 h-4 text-amber-500" /><span>{t.regions.facts.temp}</span></div>
                  <div className="text-sm font-extrabold text-slate-900">{currentCountry.temp || '+26°C'}</div>
                </div>
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/90 shadow-2xs">
                  <div className="flex items-center gap-1.5 text-slate-500 text-xs font-bold mb-1"><Shield className="w-4 h-4 text-[#10b981]" /><span>{t.regions.facts.visa}</span></div>
                  <div className="text-sm font-extrabold text-slate-900">{countryVisa}</div>
                </div>
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/90 shadow-2xs">
                  <div className="flex items-center gap-1.5 text-slate-500 text-xs font-bold mb-1"><Plane className="w-4 h-4 text-sky-600" /><span>{t.regions.facts.duration}</span></div>
                  <div className="text-xs font-extrabold text-slate-900 leading-snug">{countryFlight}</div>
                </div>
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/90 shadow-2xs">
                  <div className="flex items-center gap-1.5 text-slate-500 text-xs font-bold mb-1"><Star className="w-4 h-4 text-amber-500" /><span>{t.regions.facts.season}</span></div>
                ...
                <div className="space-y-1.5">
                  {countryHighlights?.map((h, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs font-medium text-slate-700">
                      <div className="w-4 h-4 rounded-full bg-[#ecfdf5] border border-[#a7f3d0] flex items-center justify-center text-[#10b981]"><Check className="w-2.5 h-2.5" /></div>
                      <span>{h}</span>
                    </div>
                  ))}
                </div>
