...
import { Flame, Clock, CheckCircle2, ArrowRight, Sparkles, Plane } from 'lucide-react';
      ...
      <div className="max-w-[1536px] mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 flex items-center gap-3.5 shadow-sm">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xl sm:text-2xl font-black text-slate-900 font-mono mt-1">
                ...
                <span>:</span>
                <span className="bg-amber-500 text-white px-2.5 py-1 rounded-xl">{String(timeLeft.seconds).padStart(2, '0')}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {HOT_DEALS.map((deal) => (
            <div
              key={deal.id}
              className="bg-white rounded-3xl overflow-hidden border border-slate-200 hover:border-sky-400 flex flex-col justify-between group transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-1"
            >
              <div>
                {/* Image Cover */}
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="text-[10px] font-black tracking-wider uppercase px-3 py-1 rounded-full bg-red-600 text-white shadow-md">
                      {deal.badge}
                    </span>
                  </div>
